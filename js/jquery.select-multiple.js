    ...
    SelectMultiple.prototype = {
        ...
        init: function(){
            ...
            if (ms.next('.ms-container').length === 0){
                ...
                this.$container.attr('id', 'ms-' + ms.attr('id'));//ms-my-select的id是这么来的。。
        ...
        "generateListFromOption": function (option, index, $container) {
            ...
            var selectableLi = $('<li '+attributes+'><span style="display:inline-block;width:52vw">'+(that.options.allowHTML === true ? $option.text() : that.escapeHTML($option.text()))+'</span><span class="pull-right ms-elem-selected">✔</span></li>'),
                //原来是✔符号
            ...
            selectableLi.addClass('deselect-rollcall');//新加的，未选中就变灰
            selectableLi
                .data('ms-value', value)
        ...
        'activeKeyboard' : function($list){
            ...
            $list.on('focus', function(){
                ...
                //根据http://api.jquery.com/on/，可以为一个元素设置多个同名的on，依次执行
                //添加by鸿博
                .on('keydown', function (e) {
                    that.deactiveMouse($list);//禁用鼠标
                    $list.on('mousemove', function () {
                        that.activeMouse($list);//一旦鼠标移动了，就启用鼠标
                        $list.off("mousemove");//取消对鼠标移动事件的监听
                    });
                })
                .on('keydown', function(e){
                    switch (e.which) {
                        ...
                        case 32:
                            e.preventDefault();
                            e.stopPropagation();//阻止继续派发事件
                            that.selectHighlighted($list);//虽然我也不知道为什么，反正就改了这几个地方就搞定了，详见此库issue
                            return;
                        case 9:
        ...
        'activeMouse' : function($list){
            var that = this;
            //为所有的<li>添加mouseenter事件，使得鼠标进入的时候，先清除所有的<li>的ms-hover类，再为当前的<li>添加ms-hover类
            //另外在chrome dev tool里mouseenter好像显示为mouseover
            //issue:这里把'body'改为了$list，否则<body>里的li都会被影响
            $list.on('mouseenter', that.elemsSelector, function () {
        ...
        //添加by 鸿博 用于在键盘控制时禁止鼠标来防止错乱
        'deactiveMouse': function ($list) {
            var that = this;
            $list.off('mouseenter', that.elemsSelector);
        },
        ...
        'select' : function(value, method){
            ...
            if (selectables.length > 0){
                ...
                selectables.removeClass('deselect-rollcall');//新加的，选中就变黑
        ...
        'deselect' : function(value, method){
            ...
            selectables.addClass('deselect-rollcall');//新加的，取消选中就变灰
        ...
        'deselect_all' : function(){
            ...
            //修改by鸿博 批量deselect时 循环触发afterDeselect
            //:first-child取所有的ms-selected的第一个孩子 :first只取一个就停止
            this.$selectableUl.find('.ms-selected span:first-child').each(function () {
                var value = $(this).text(); //此处不能用传进来的index,val 要用$(this)
                if (typeof that.options.afterDeselect === 'function') {
                    that.options.afterDeselect.call(that, value);
                }
            });
